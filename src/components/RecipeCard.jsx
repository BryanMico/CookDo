...
const RecipeCard = ({recipe, bg, badge}) => {
    ...
    const addRecipeToFav = () => {
        ...
        const isRecipeAlreadyFav = favorites.some((fav)=> fav.label === recipe.label);
    ...
    const healthLabels = getTwoValFromArr(recipe.healthLabels);
  return (
...
<a href={`https://www.youtube.com/results?search_query=${recipe.label} recipe`} target="_blank" className="relative h-32">
  ...
  <img src= {recipe.image} alt="recipe img" className="rounded-md w-full h-full object-cover cursor-pointer opacity-0 transition-opacity duration-300" onLoad={(e)=> {e.currentTarget.style.opacity = 1; e.currentTarget.previousElementSibling.style.display = "none"}}/>
  <div className="absolute bottom-2 left-2  bg-white rounded-full p-1 cursor-pointer flex items-center
  gap-1 text-sm"> <Soup size={16}/>{recipe.yield} Servings</div>
